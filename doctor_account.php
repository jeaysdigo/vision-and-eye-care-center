...
$first_name = $last_name = $email = $password = $bday = $gender = $contact = $specialization = $address = $municipality = $city = $zipcode = "";
...
if (isset($doctorId)) {
    ...
    $stmt = $conn->prepare("SELECT FirstName, LastName, Email, DateOfBirth, Gender, ContactNumber, Specialization, Address, Municipality, City,Zipcode FROM doctors WHERE doctorID = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $stmt->bind_result($first_name, $last_name, $email, $bday, $gender, $contact, $specialization, $address, $municipality, $city, $zipcode);
...
?>
    ...
    <input type="hidden" name="patient_id" value="<?php echo $doctorId ?>">
        <div>
