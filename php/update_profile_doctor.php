...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['patient_id'])) {
    ...
    $stmt_check_email2->bind_param("si", $email, $patient_id);
    ...
    $stmt_check_contact2->bind_param("si", $contact, $patient_id);
    ...
    if ($result_email->num_rows > 0) {
        ...
        $stmt_update = $conn->prepare("UPDATE doctors SET FirstName=?, LastName=?, Email=?, DateOfBirth=?, Gender=?, ContactNumber=?, Address=?, Municipality=?, City=?, ZipCode=? WHERE DoctorID=?");
        $stmt_update->bind_param("ssssssssssi", $first_name, $last_name, $email, $bday, $gender, $contact, $address, $municipality, $city, $zipcode, $patient_id);
    ...
    $result_email2->close();
    $stmt_check_email2->close();
    $result_contact2->close();
    $stmt_check_contact2->close();
}
