...
$sql = "SELECT appointments.AppointmentID, appointments.AppointmentDate, appointments.Notes, appointments.Status,
        ...
        services.ServiceName, doctors.FirstName AS DoctorFirstName, doctors.LastName AS DoctorLastName
        FROM appointments
        ...
        <tbody id="table">
                    <?php if (!empty($appointments)): ?>
                        <?php foreach ($appointments as $appointment): ?>
                            ...
                            <td class="px-6 py-4">
                            <?= date("g:i A - F j, Y", strtotime($appointment["AppointmentDate"])); ?>
                            

                                </td>
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <?= htmlspecialchars($appointment['FirstName']) . " " .  htmlspecialchars($appointment['LastName']) ?>
                                </th>
                                <!-- <td class="px-6 py-4">
                                    <?= htmlspecialchars($appointment['ContactNumber']) ?>
                                </td> -->
                                <!-- <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <?= htmlspecialchars($appointment['ServiceName']) ?>
                                </td>
                                <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <?= htmlspecialchars($appointment['DoctorFirstName']) . " " .  htmlspecialchars($appointment['DoctorLastName']) ?>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <button type="button" data-appointment-id="<?= htmlspecialchars($appointment['AppointmentID']) ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
    ...
    $('[data-appointment-id]').click(function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var serviceId = deleteButton.data('appointment-id');

                // Make an AJAX request to delete the service
                $.ajax({
                    url: 'php/delete_book.php', // Replace with your PHP script to handle deletion
                    method: 'POST',
                    data: { service_id: serviceId },
                    success: function (response) {
                        console.log();
